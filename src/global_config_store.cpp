...
const QStringList kConfigNames = {
    QStringLiteral(".el-baton.yml"), QStringLiteral(".el-baton.yaml"), QStringLiteral(".el-baton.json"),
    QStringLiteral(".notable.yml"), QStringLiteral(".notable.yaml"), QStringLiteral(".notable.json"),
    QStringLiteral("config.yml"), QStringLiteral("config.yaml"), QStringLiteral("config.json")};

QVariant fromYaml(const YAML::Node& node) {
  if (!node || node.IsNull()) return {};
  if (node.IsSequence()) {
    QVariantList values;
    for (const YAML::Node& child : node) values.append(fromYaml(child));
    return values;
  }
  if (node.IsMap()) {
    QVariantMap values;
    for (const auto& entry : node) values.insert(QString::fromStdString(entry.first.as<std::string>()), fromYaml(entry.second));
    return values;
  }
  const QString scalar = QString::fromStdString(node.Scalar());
  if (scalar.compare(QStringLiteral("true"), Qt::CaseInsensitive) == 0) return true;
  if (scalar.compare(QStringLiteral("false"), Qt::CaseInsensitive) == 0) return false;
...
QVariantMap mergeMaps(QVariantMap base, const QVariantMap& overrides) {
  for (auto it = overrides.cbegin(); it != overrides.cend(); ++it) {
    if (base.value(it.key()).metaType().id() == QMetaType::QVariantMap &&
      ...
      base.insert(it.key(), mergeMaps(base.value(it.key()).toMap(), it.value().toMap()));
    } else {
...
QVariantMap setNested(QVariantMap map, const QStringList& parts, qsizetype index, const QVariant& value) {
  if (index == parts.size() - 1) {
    map.insert(parts.at(index), value);
    return map;
  }
  map.insert(parts.at(index), setNested(map.value(parts.at(index)).toMap(), parts, index + 1, value));
  return map;
}
...
QVariantMap GlobalConfigStore::defaults() {
  return {
      ...
      {QStringLiteral("performance"), QVariantMap{{QStringLiteral("highPerformanceMode"), false}}},
      {QStringLiteral("battery"), QVariantMap{{QStringLiteral("enabled"), false}, {QStringLiteral("autoDetect"), true},
          {QStringLiteral("targetFps"), 30}, {QStringLiteral("optimizeRendering"), true},
          {QStringLiteral("renderDelayMs"), 400}, {QStringLiteral("disableSpellcheck"), false},
          {QStringLiteral("disableAutocomplete"), false}, {QStringLiteral("disableAnimations"), true}}},
      {QStringLiteral("spellcheck"), QVariantMap{{QStringLiteral("addedWords"), QVariantList()}, {QStringLiteral("disable"), false}}},
      {QStringLiteral("notes"), QVariantMap{{QStringLiteral("disableAutomaticRenaming"), false}}},
      {QStringLiteral("ui"), QVariantMap{{QStringLiteral("disableAnimations"), false}}},
      {QStringLiteral("input"), QVariantMap{{QStringLiteral("disableMiddleClickPaste"), false}}},
      {QStringLiteral("preview"), QVariantMap{{QStringLiteral("largeNoteFullRenderDelay"), 500},
          {QStringLiteral("disableScriptSanitization"), false}, {QStringLiteral("disableSplitViewSync"), false}}},
      {QStringLiteral("monaco"), QVariantMap{{QStringLiteral("tableFormattingDelay"), 2000},
          {QStringLiteral("disableAutomaticTableFormatting"), false},
          {QStringLiteral("editorOptions"), QVariantMap{{QStringLiteral("lineNumbers"), QStringLiteral("on")},
              {QStringLiteral("disableSuggestions"), false}, {QStringLiteral("tabSize"), 2}}}}},
      {QStringLiteral("plantuml"), QVariantMap{{QStringLiteral("externalServerUrl"), QString()},
          {QStringLiteral("requestTimeoutMs"), 12000}, {QStringLiteral("cacheMaxEntries"), 400},
...
void GlobalConfigStore::setWorkspaceRoot(const QString& workspaceRoot) {
  ...
  if (filePath_.isEmpty() && !workspaceRoot_.isEmpty()) filePath_ = root.filePath(QStringLiteral(".el-baton.yml"));
  (void)reload();
}

bool GlobalConfigStore::reload(QString* errorMessage) {
  values_ = defaults();
  if (filePath_.isEmpty() || !QFileInfo::exists(filePath_)) return true;
  ...
  try {
    ...
    QVariantMap parsed;
    if (QFileInfo(filePath_).suffix().compare(QStringLiteral("json"), Qt::CaseInsensitive) == 0) {
...
bool GlobalConfigStore::save(QString* errorMessage) const {
  if (filePath_.isEmpty()) {
    if (errorMessage != nullptr) *errorMessage = QStringLiteral("No workspace data directory is selected.");
    return false;
  }
  QByteArray content;
  if (QFileInfo(filePath_).suffix().compare(QStringLiteral("json"), Qt::CaseInsensitive) == 0) {
    content = QJsonDocument::fromVariant(values_).toJson(QJsonDocument::Indented);
  } else {
    YAML::Emitter emitter;
    emitter.SetIndent(2);
    emitter << toYaml(values_);
    content = QByteArray(emitter.c_str(), static_cast<qsizetype>(emitter.size())) + '\n';
  }
...
QVariant GlobalConfigStore::value(const QString& dottedKey, const QVariant& fallback) const {
  ...
  for (const QString& part : dottedKey.split(QLatin1Char('.'), Qt::SkipEmptyParts)) {
    ...
    if (!map.contains(part)) return fallback;
    current = map.value(part);
  }
  return current;
}

void GlobalConfigStore::setValue(const QString& dottedKey, const QVariant& value) {
  const QStringList parts = dottedKey.split(QLatin1Char('.'), Qt::SkipEmptyParts);
  if (!parts.isEmpty()) values_ = setNested(values_, parts, 0, value);
}
