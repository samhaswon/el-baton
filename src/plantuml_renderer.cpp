...
QString PlantUmlRenderer::normalizeSource(const QString& rawSource) {
  ...
  static const QRegularExpression startExpression(QStringLiteral("^@start(\\w+)?\\b"),
                                                   QRegularExpression::CaseInsensitiveOption);
  static const QRegularExpression endExpression(QStringLiteral("^@end(\\w+)?\\b"),
                                                 QRegularExpression::CaseInsensitiveOption);
  const QRegularExpressionMatch start = startExpression.match(lines.at(first).trimmed());
  const QRegularExpressionMatch end = endExpression.match(lines.at(last).trimmed());
  if (start.hasMatch() && end.hasMatch()) return source;

  const QString suffix = !start.captured(1).isEmpty() ? start.captured(1).toLower()
      : !end.captured(1).isEmpty() ? end.captured(1).toLower() : QStringLiteral("uml");
  QStringList output = lines;
  if (!start.hasMatch()) output.prepend(QStringLiteral("@start") + suffix);
  if (!end.hasMatch()) output.append(QStringLiteral("@end") + suffix);
...
QString PlantUmlRenderer::normalizeLocalError(const QString& rawMessage) {
  ...
  const bool graphvizMissing = lower.contains(QStringLiteral("cannot find graphviz")) ||
      (lower.contains(QStringLiteral("graphviz")) && lower.contains(QStringLiteral("not found"))) ||
      lower.contains(QStringLiteral("dot executable")) || lower.contains(QStringLiteral("failed to execute dot")) ||
      lower.contains(QStringLiteral("testdot"));
  return graphvizMissing
      ? QStringLiteral("Graphviz is required for local PlantUML rendering but was not found.")
      : message;
}
...
QString PlantUmlRenderer::normalizeServerUrl(const QString& rawUrl) {
  ...
  if (!url.isValid() || url.host().isEmpty() ||
      (url.scheme() != QStringLiteral("http") && url.scheme() != QStringLiteral("https"))) return {};
...
QString PlantUmlRenderer::buildRemoteRenderUrl(const QString& serverUrl) {
  ...
  QString path = url.path();
  while (path.endsWith(QLatin1Char('/'))) path.chop(1);
  static const QRegularExpression payload(QStringLiteral("^(.*)/svg/[^/]+$"),
                                           QRegularExpression::CaseInsensitiveOption);
...
QString PlantUmlRenderer::buildRemoteSvgUrl(const QString& serverUrl, const QString& encodedDiagram) {
...
QString PlantUmlRenderer::encodeForServer(const QString& source) {
  ...
  z_stream stream{};
  if (deflateInit2(&stream, 9, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY) != Z_OK) return {};
  QByteArray compressed;
  compressed.resize(static_cast<qsizetype>(deflateBound(&stream, static_cast<uLong>(input.size()))));
  ...
  const auto encode6Bit = [](int value) -> QChar {
    if (value < 10) return QChar('0' + value);
    value -= 10;
    if (value < 26) return QChar('A' + value);
    value -= 26;
    if (value < 26) return QChar('a' + value);
  ...
  for (qsizetype index = 0; index < compressed.size(); index += 3) {
    ...
    const int second = index + 1 < compressed.size() ? static_cast<unsigned char>(compressed.at(index + 1)) : 0;
    const int third = index + 2 < compressed.size() ? static_cast<unsigned char>(compressed.at(index + 2)) : 0;
...
void PlantUmlRenderer::beginBatch(const QJsonObject& batchObject) {
  Batch batch;
  batch.generation = batchObject.value(QStringLiteral("generation")).toVariant().toULongLong();
  const QJsonArray requests = batchObject.value(QStringLiteral("requests")).toArray();
  ...
  for (qsizetype index = 0; index < requests.size() && index < 64; ++index) {
    ...
    batch.requests.append({request.value(QStringLiteral("id")).toString(),
...
void PlantUmlRenderer::advanceBatch() {
  if (!currentBatch_.has_value() || process_ != nullptr) return;
  ...
  if (request.source.isEmpty()) {
    appendResult(request, {{QStringLiteral("ok"), false},
                           {QStringLiteral("error"), QStringLiteral("Empty PlantUML source")}});
    advanceBatch();
    return;
  }
...
void PlantUmlRenderer::startProcess(const Request& request) {
  if (!QFileInfo(jarPath_).isFile()) {
    const QJsonObject result{{QStringLiteral("ok"), false},
                             {QStringLiteral("error"), QStringLiteral("Local PlantUML JAR was not found: %1").arg(jarPath_)}};
  ...
  connect(processTimer_, &QTimer::timeout, this, [this, launchedProcess] {
    if (process_ != launchedProcess) return;
  ...
  connect(launchedProcess, &QProcess::errorOccurred, this,
    ...
    if (error != QProcess::FailedToStart || process_ != launchedProcess) return;
    finishProcess(launchedProcess,
                  {{QStringLiteral("ok"), false},
                   {QStringLiteral("error"), QStringLiteral("Failed to launch Java: %1").arg(launchedProcess->errorString())}});
  });
  connect(launchedProcess, qOverload<int, QProcess::ExitStatus>(&QProcess::finished), this,
    ...
    const QString errors = QString::fromUtf8(launchedProcess->readAllStandardError()).trimmed();
    static const QRegularExpression svg(QStringLiteral("<svg(?:\\s|>)"),
                                        QRegularExpression::CaseInsensitiveOption);
    if (svg.match(output).hasMatch()) {
      finishProcess(launchedProcess, {{QStringLiteral("ok"), true}, {QStringLiteral("svg"), output}});
      return;
    }
    QString message = processTimedOut_
        ? QStringLiteral("Local renderer timed out after %1ms").arg(timeoutMs_)
        : (!errors.isEmpty() ? errors : output);
    if (message.isEmpty()) {
      message = QStringLiteral("PlantUML exited with code %1 without returning SVG output").arg(exitCode);
    }
    finishProcess(launchedProcess,
                  {{QStringLiteral("ok"), false}, {QStringLiteral("error"), normalizeLocalError(message)}});
  });
  connect(launchedProcess, &QProcess::started, this, [this, launchedProcess, request] {
    if (process_ != launchedProcess) return;
...
void PlantUmlRenderer::finishProcess(QProcess* process, const QJsonObject& result) {
  if (process_ != process || !activeRequest_.has_value()) return;
  const Request request = *activeRequest_;
  if (processTimer_ != nullptr) processTimer_->stop();
  process_ = nullptr;
...
void PlantUmlRenderer::continueWithRemote(const Request& request, const QJsonObject& localResult) {
  if (externalServerUrl_.isEmpty()) {
    finishRequest(request, localResult);
    return;
  }
  const QString remoteKey = QStringLiteral("remote\0") + externalServerUrl_ + QChar::Null + request.source;
...
void PlantUmlRenderer::startRemoteRequest(const Request& request, bool encodedGet) {
  QNetworkRequest networkRequest;
  networkRequest.setAttribute(QNetworkRequest::RedirectPolicyAttribute, QNetworkRequest::NoLessSafeRedirectPolicy);
  networkRequest.setRawHeader("Accept", "image/svg+xml,text/plain;q=0.9,*/*;q=0.1");
  if (encodedGet) {
    networkRequest.setUrl(QUrl(buildRemoteSvgUrl(externalServerUrl_, encodeForServer(request.source))));
    networkReply_ = network_->get(networkRequest);
  } else {
    networkRequest.setUrl(QUrl(buildRemoteRenderUrl(externalServerUrl_)));
    networkRequest.setHeader(QNetworkRequest::ContentTypeHeader, QStringLiteral("text/plain; charset=utf-8"));
  ...
  connect(reply, &QNetworkReply::downloadProgress, this, [reply](qint64 received, qint64 total) {
    if (received > 32 * 1024 * 1024 || total > 32 * 1024 * 1024) reply->abort();
  });
  connect(reply, &QNetworkReply::finished, this, [this, reply] { finishRemoteRequest(reply); });
...
void PlantUmlRenderer::finishRemoteRequest(QNetworkReply* reply) {
  if (networkReply_ != reply || !activeRequest_.has_value()) return;
  if (networkTimer_ != nullptr) networkTimer_->stop();
  ...
  if (successful) {
    remoteResult = {{QStringLiteral("ok"), true}, {QStringLiteral("svg"), text},
                    {QStringLiteral("externalUrl"), buildRemoteSvgUrl(externalServerUrl_, encodeForServer(request.source))}};
  } else {
    QString error = text;
    if (error.isEmpty()) error = reply->errorString();
    if (error.isEmpty()) error = QStringLiteral("Remote renderer failed with HTTP %1").arg(status);
    remoteResult = {{QStringLiteral("ok"), false}, {QStringLiteral("error"), error},
                    {QStringLiteral("externalUrl"), buildRemoteSvgUrl(externalServerUrl_, encodeForServer(request.source))}};
  }
  remember(QStringLiteral("remote\0") + externalServerUrl_ + QChar::Null + request.source, remoteResult);
...
void PlantUmlRenderer::finishRequest(const Request& request, const QJsonObject& localResult,
                                     const std::optional<QJsonObject>& remoteResult) {
  const bool localOk = localResult.value(QStringLiteral("ok")).toBool();
  const bool remoteOk = remoteResult.has_value() && remoteResult->value(QStringLiteral("ok")).toBool();
  const bool selectedRemote = remoteOk || (!localOk && remoteResult.has_value());
  QJsonObject selected = selectedRemote ? *remoteResult : localResult;
  selected.insert(QStringLiteral("origin"), selectedRemote ? QStringLiteral("remote") : QStringLiteral("local"));
  selected.insert(QStringLiteral("localStatus"), localOk ? QStringLiteral("ok") : QStringLiteral("error"));
  if (!localOk) selected.insert(QStringLiteral("localError"), localResult.value(QStringLiteral("error")));
  if (remoteResult.has_value()) {
    selected.insert(QStringLiteral("remoteStatus"), remoteOk ? QStringLiteral("ok") : QStringLiteral("error"));
    if (!remoteOk) selected.insert(QStringLiteral("remoteError"), remoteResult->value(QStringLiteral("error")));
  }
  appendResult(request, selected);
  advanceBatch();
}

void PlantUmlRenderer::appendResult(const Request& request, const QJsonObject& rawResult) {
...
void PlantUmlRenderer::finishBatch() {
  QJsonObject result{{QStringLiteral("generation"), static_cast<qint64>(currentBatch_->generation)},
                     {QStringLiteral("results"), currentBatch_->results},
                     {QStringLiteral("plantUmlMs"), currentBatch_->elapsed.nsecsElapsed() / 1'000'000.0}};
  currentBatch_.reset();
...
void PlantUmlRenderer::remember(const QString& source, const QJsonObject& result) {
