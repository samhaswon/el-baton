...
int main(int argc, char *argv[]) {
  for (int index = 1; index < argc; ++index) {
    if (QByteArray(argv[index]) != QByteArrayLiteral("--unthrottled-webengine"))
      continue;
  ...
  parser.addPositionalArgument(QStringLiteral("file"),
                               QStringLiteral("Markdown note to open."),
                               QStringLiteral("[file]"));
  qt_editor::addRuntimeOptions(parser);
  parser.addOption({QStringLiteral("diagnostics"),
                    QStringLiteral("Show the preview performance overlay.")});
  parser.addOption({QStringLiteral("no-diagnostics"),
                    QStringLiteral("Hide all rendering diagnostics.")});
  parser.process(application);

  QString pathError;
  const el_baton::ApplicationPaths paths = el_baton::ApplicationPaths::system();
  if (!paths.ensureCreated(&pathError)) {
    QMessageBox::critical(nullptr, QObject::tr("El Baton startup failed"),
                          pathError);
    return 1;
  }

  qt_editor::BenchmarkOptions options =
      qt_editor::parseBenchmarkOptions(parser, false);
#ifdef NDEBUG
  ...
  if (parser.isSet(QStringLiteral("diagnostics")))
    options.overlayEnabled = true;
  if (parser.isSet(QStringLiteral("no-diagnostics")))
    options.overlayEnabled = false;
