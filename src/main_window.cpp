...
@media print {
  ...
  const QString printScripts = printLayout
      ? QStringLiteral("<script src=\"%1\"></script>").arg(
            QUrl::fromLocalFile(webAssetPath(QStringLiteral("vendor/mermaid.min.js")))
...
MainWindow::MainWindow(BenchmarkOptions options, QWidget* parent)
    : QMainWindow(parent), options_(options), pipeline_(new MarkdownPipeline), bridge_(new PreviewBridge(this)),
      plantUmlRenderer_(new PlantUmlRenderer(el_baton::ApplicationPaths::plantUmlJar(
          QStringLiteral(QT_EDITOR_PLANTUML_JAR)), this)),
  ...
  emojiCompletions_ = MarkdownCompletion::loadEmojiMap(generatedDataPath(
      QStringLiteral("emoji-shortcodes.json"), QStringLiteral(QT_EDITOR_EMOJI_JSON)));
...
QWidget* MainWindow::createHelpPanel() {
  ...
  connect(helpBridge, &PreviewBridge::clientReady, content,
    ...
    if (role != QStringLiteral("preview")) return;
    QFile file(generatedDataPath(
        QStringLiteral("cheatsheet.md"), QStringLiteral(QT_EDITOR_CHEATSHEET_MARKDOWN)));
    if (!file.open(QIODevice::ReadOnly)) {
  ...
  panel->setFirstShowCallback([content] {
    content->setUrl(QUrl::fromLocalFile(webAssetPath(QStringLiteral("preview.html"))));
  });
...
void MainWindow::configurePreview() {
  ...
  preview_->setUrl(QUrl::fromLocalFile(webAssetPath(QStringLiteral("preview.html"))));
  if (options_.hiddenMermaidPage) {
    ...
    hiddenMermaidPage_->setUrl(
        QUrl::fromLocalFile(webAssetPath(QStringLiteral("mermaid_renderer.html"))));
  }
}
