...
WorkspaceGraphView::WorkspaceGraphView(QWidget *parent) : QWidget(parent) {
  ...
  connect(&simulationTimer_, &QTimer::timeout, this,
          &WorkspaceGraphView::simulationStep);
}
...
void WorkspaceGraphView::setGraph(const WorkspaceGraph &graph) {
  ...
  for (const NodeState &state : nodes_)
    oldPositions.insert(state.node.id, state.position);
  ...
  for (const WorkspaceGraphEdge &edge : edges_) {
    ...
    if (source != nodeIndexes_.cend())
      ++nodes_[source.value()].connections;
    if (target != nodeIndexes_.cend())
      ++nodes_[target.value()].connections;
  }
  selectedNode_ = -1;
  hoveredNode_ = -1;
  rebuildVisibility();
...
void WorkspaceGraphView::setNodeKindsVisible(bool notes, bool tags,
                                             bool attachments) {
  notesVisible_ = notes;
  tagsVisible_ = tags;
...
void WorkspaceGraphView::reheat() {
  ...
  simulationTimer_.setInterval(visibleCount > 1200  ? 30
                               : visibleCount > 500 ? 22
                                                    : 16);
...
void WorkspaceGraphView::fitToView() {
  ...
  const double horizontal =
      static_cast<double>(width()) / std::max(1.0, bounds.width());
  const double vertical =
      static_cast<double>(height()) / std::max(1.0, bounds.height());
...
void WorkspaceGraphView::rebuildVisibility() {
  ...
  for (int index = 0; index < nodes_.size(); ++index) {
    ...
    const bool kindVisible =
        state.node.kind == WorkspaceGraphNodeKind::Note  ? notesVisible_
        : state.node.kind == WorkspaceGraphNodeKind::Tag ? tagsVisible_
                                                         : attachmentsVisible_;
    const bool linkVisible =
        state.connections > 0 ? linkedVisible_ : unlinkedVisible_;
  ...
  for (int edgeIndex = 0; edgeIndex < edges_.size(); ++edgeIndex) {
    ...
    if (source != nodeIndexes_.cend() && target != nodeIndexes_.cend() &&
        nodes_.at(source.value()).visible &&
        nodes_.at(target.value()).visible) {
      ++visibleEdges;
      visibleEdgeIndexes_.append(edgeIndex);
    }
  }
  if (selectedNode_ >= 0 && !nodes_.at(selectedNode_).visible)
    selectedNode_ = -1;
...
void WorkspaceGraphView::simulationStep() {
  const int visibleCount = static_cast<int>(visibleNodeIndexes_.size());
  const int stepLimit = visibleCount > 1500  ? 80
                        : visibleCount > 800 ? 120
                        : visibleCount > 300 ? 180
                                             : 260;
  ...
  const int sampleLimit = visibleCount > 1500  ? 8
                          : visibleCount > 800 ? 12
                          : visibleCount > 300 ? 24
                                               : 48;
  ...
  for (int visibleIndex = 0; visibleIndex < visibleCount; ++visibleIndex) {
    ...
    for (int sample = 1; sample <= samples; ++sample) {
      ...
      const double collision =
          std::max(0.0, collisionRadius_ - distance) * 0.24;
      const double repulsion =
          repulsionStrength_ / std::max(36.0, distance * distance);
  ...
  for (const int edgeIndex : visibleEdgeIndexes_) {
    ...
    if (!nodes_.at(sourceIndex).visible || !nodes_.at(targetIndex).visible)
      continue;
    const QPointF delta =
        nodes_.at(targetIndex).position - nodes_.at(sourceIndex).position;
  ...
  for (const int index : visibleNodeIndexes_) {
    ...
    if (speed > 9.0)
      state.velocity *= 9.0 / speed;
    state.position += state.velocity;
    energy += vectorLength(state.velocity);
  }
  if (simulationSteps_ > 30 &&
      energy < static_cast<double>(visibleCount) * 0.02) {
    simulationTimer_.stop();
    simulationPending_ = false;
  }
  update();
...
int WorkspaceGraphView::nodeAt(const QPointF &screen) const {
  ...
  for (const int index : visibleNodeIndexes_) {
    const double distance =
        vectorLength(toScreen(nodes_.at(index).position) - screen);
...
QColor WorkspaceGraphView::nodeColor(const WorkspaceGraphNodeKind kind) const {
  if (kind == WorkspaceGraphNodeKind::Tag)
    return QColor(QStringLiteral("#ff5d62"));
  if (kind == WorkspaceGraphNodeKind::Attachment)
    return QColor(QStringLiteral("#38a7e8"));
...
void WorkspaceGraphView::paintEvent(QPaintEvent *event) {
  ...
  for (const int edgeIndex : visibleEdgeIndexes_) {
    ...
    painter.drawLine(toScreen(sourceState.position),
                     toScreen(targetState.position));
  }

  for (const int index : visibleNodeIndexes_) {
    const NodeState &state = nodes_.at(index);
    const QPointF center = toScreen(state.position);
    const double radius = index == selectedNode_  ? 6.5
                          : index == hoveredNode_ ? 5.5
                                                  : 3.2;
  ...
  if (labelIndex >= 0 && labelIndex < nodes_.size() &&
      nodes_.at(labelIndex).visible) {
    ...
    const QRect textBounds =
        metrics.boundingRect(state.node.label).adjusted(-7, -4, 7, 4);
...
void WorkspaceGraphView::mouseDoubleClickEvent(QMouseEvent *event) {
  const int hit = nodeAt(event->position());
  if (hit >= 0)
    emit nodeActivated(nodes_.at(hit).node);
  event->accept();
}
