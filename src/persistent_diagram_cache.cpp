...
bool PersistentDiagramCache::ensureOpen() {
  ...
  for (const QString& statement : {
           ...
           QStringLiteral("CREATE TABLE IF NOT EXISTS diagram_cache ("
...
std::optional<QJsonObject> PersistentDiagramCache::get(const QString& key) {
  ...
  QSqlQuery query(database);
  query.prepare(QStringLiteral("SELECT payload FROM diagram_cache WHERE cache_key = ?"));
  ...
  touch.prepare(QStringLiteral("UPDATE diagram_cache SET last_accessed = ? WHERE cache_key = ?"));
...
bool PersistentDiagramCache::put(const QString& key, const QJsonObject& value) {
  ...
  const QByteArray payload = qCompress(QJsonDocument(value).toJson(QJsonDocument::Compact), 6);
  QSqlQuery query(QSqlDatabase::database(connectionName_));
  query.prepare(QStringLiteral(
      "INSERT INTO diagram_cache(cache_key,payload,size_bytes,last_accessed) VALUES(?,?,?,?) "
...
bool PersistentDiagramCache::prune() {
  ...
  if (!stats.exec(QStringLiteral(
          "SELECT COUNT(*),COALESCE(SUM(size_bytes),0) FROM diagram_cache")) || !stats.next()) return false;
  ...
  while (count > maxEntries_ || bytes > maxBytes_) {
    ...
    if (!remove.exec(QStringLiteral(
            "DELETE FROM diagram_cache WHERE cache_key IN (SELECT cache_key FROM diagram_cache "
            "ORDER BY last_accessed ASC LIMIT 1)"))) return false;
    if (!stats.exec(QStringLiteral(
            "SELECT COUNT(*),COALESCE(SUM(size_bytes),0) FROM diagram_cache")) || !stats.next()) return false;
