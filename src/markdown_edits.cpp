...
bool hasUnescapedPipe(const QString& line) {
  for (qsizetype index = 0; index < line.size(); ++index) {
    ...
    if (index > 0 && line.at(index - 1) == QLatin1Char('\\')) continue;
    return true;
  }
  return false;
}
...
std::optional<QPair<QChar, qsizetype>> fenceAt(const QString& line) {
  static const QRegularExpression expression(QStringLiteral("^\\s*(`{3,}|~{3,})"));
  const QRegularExpressionMatch match = expression.match(line);
  if (!match.hasMatch()) return std::nullopt;
  return QPair<QChar, qsizetype>{match.captured(1).at(0), match.capturedLength(1)};
}
...
bool isTableCandidate(const QStringList& lines, int line) {
  if (line < 0 || line >= lines.size() || lines.at(line).trimmed().isEmpty()) return false;
  if (fenceAt(lines.at(line)).has_value() || isInsideFence(lines, line)) return false;
...
QStringList splitTableRow(const QString& line, bool* valid = nullptr) {
  ...
  if (trimmed.startsWith(QLatin1Char('|'))) cells.removeFirst();
  if (trimmed.endsWith(QLatin1Char('|'))) cells.removeLast();
...
std::optional<TableAlignment> tableAlignment(const QString& cell) {
  ...
  if (!delimiter.match(value).hasMatch()) return std::nullopt;
...
std::optional<TableBlock> tableBlockAt(const QStringList& lines, int line) {
  if (!isTableCandidate(lines, line)) return std::nullopt;
  int start = line;
  int end = line;
  while (isTableCandidate(lines, start - 1)) --start;
  while (isTableCandidate(lines, end + 1)) ++end;
  for (int candidate = start; candidate < end; ++candidate) {
    bool headerValid = false;
    bool delimiterValid = false;
    splitTableRow(lines.at(candidate), &headerValid);
    const QStringList delimiter = splitTableRow(lines.at(candidate + 1), &delimiterValid);
    if (!headerValid || !delimiterValid || !isDelimiterRow(delimiter)) continue;
    ...
    if (line >= candidate && line <= candidateEnd) return TableBlock{candidate, candidateEnd};
  }
  return std::nullopt;
}

QString paddedCell(const QString& value, qsizetype width, TableAlignment alignment) {
  const qsizetype padding = std::max<qsizetype>(0, width - value.size());
  if (alignment == TableAlignment::Right) return QString(padding, QLatin1Char(' ')) + value;
...
QString delimiterCell(qsizetype width, TableAlignment alignment) {
  ...
  if (alignment == TableAlignment::Left) return QLatin1Char(':') + QString(finalWidth - 1, QLatin1Char('-'));
  if (alignment == TableAlignment::Right) return QString(finalWidth - 1, QLatin1Char('-')) + QLatin1Char(':');
  if (alignment == TableAlignment::Center) {
    return QLatin1Char(':') + QString(finalWidth - 2, QLatin1Char('-')) + QLatin1Char(':');
  }
  return QString(finalWidth, QLatin1Char('-'));
}

QString formatTableBlock(const QStringList& inputLines) {
  if (inputLines.size() < 2) return inputLines.join(QLatin1Char('\n'));
  static const QRegularExpression leadingWhitespace(QStringLiteral("^\\s*"));
  const QString indent = leadingWhitespace.match(inputLines.constFirst()).captured(0);
  QVector<QStringList> rows;
  int columns = 0;
  for (const QString& sourceLine : inputLines) {
    QString line = sourceLine.startsWith(indent)
        ? sourceLine.sliced(indent.size()) : sourceLine.trimmed();
    bool valid = false;
    QStringList cells = splitTableRow(line, &valid);
    if (!valid) return inputLines.join(QLatin1Char('\n'));
    columns = std::max(columns, static_cast<int>(cells.size()));
    rows.append(std::move(cells));
  }
  if (rows.isEmpty() || !isDelimiterRow(rows.at(1))) return inputLines.join(QLatin1Char('\n'));
  for (QStringList& row : rows) {
    while (row.size() < columns) row.append(QString());
    while (row.size() > columns) row.removeLast();
  }
  QVector<TableAlignment> alignments(columns, TableAlignment::Default);
  QVector<qsizetype> widths(columns, 3);
  for (int column = 0; column < columns; ++column) {
    alignments[column] = tableAlignment(rows.at(1).at(column)).value_or(TableAlignment::Default);
    for (int row = 0; row < rows.size(); ++row) {
      if (row == 1) continue;
      widths[column] = std::max(widths.at(column), rows.at(row).at(column).size());
    }
  }
  QStringList output;
  for (int row = 0; row < rows.size(); ++row) {
    ...
    for (int column = 0; column < columns; ++column) {
      cells.append(row == 1
          ...
          : paddedCell(rows.at(row).at(column), widths.at(column), alignments.at(column)));
    }
    output.append(indent + QStringLiteral("| ") + cells.join(QStringLiteral(" | ")) + QStringLiteral(" |"));
  }
  return output.join(QLatin1Char('\n'));
}

struct CellSpan final { qsizetype start = 0; qsizetype end = 0; };

QVector<CellSpan> tableCellSpans(const QString& line) {
  QVector<qsizetype> separators;
  for (qsizetype index = 0; index < line.size(); ++index) {
    if (line.at(index) == QLatin1Char('|') &&
        (index == 0 || line.at(index - 1) != QLatin1Char('\\'))) separators.append(index);
  }
  ...
  const QString trimmed = line.trimmed();
  if (trimmed.startsWith(QLatin1Char('|')) && !spans.isEmpty()) spans.removeFirst();
  if (trimmed.endsWith(QLatin1Char('|')) && !spans.isEmpty()) spans.removeLast();
  for (CellSpan& span : spans) {
    while (span.start < span.end && line.at(span.start).isSpace()) ++span.start;
    while (span.end > span.start && line.at(span.end - 1).isSpace()) --span.end;
  }
  return spans;
}
...
qsizetype lineStartOffset(const QStringList& lines, int line) {
  qsizetype offset = 0;
  for (int index = 0; index < line && index < lines.size(); ++index) offset += lines.at(index).size() + 1;
  return offset;
}

qsizetype mapOffsetInTable(const QString& before, const QString& after, qsizetype offset) {
  ...
  const qsizetype column = std::clamp<qsizetype>(offset - lineStart, 0, beforeLines.at(line).size());
  ...
  if (beforeSpans.isEmpty() || afterSpans.isEmpty()) {
    return lineStartOffset(afterLines, line) + std::min(column, afterLines.at(line).size());
  }
  int cell = beforeSpans.size() - 1;
  for (int index = 0; index < beforeSpans.size(); ++index) {
    if (column <= beforeSpans.at(index).end) { cell = index; break; }
  }
  ...
  const qsizetype contentOffset = std::clamp<qsizetype>(column - from.start, 0, from.end - from.start);
  return lineStartOffset(afterLines, line) + to.start + std::min(contentOffset, to.end - to.start);
}
...
QString MarkdownEdits::toggleTaskLine(const QString& line, bool toggleDone) {
  static const QRegularExpression plain(QStringLiteral("^(\\s*)([*+-]?\\s*)(.*)$"));
  static const QRegularExpression unchecked(QStringLiteral("^(\\s*)([*+-]\\s+\\[ \\]\\s*)(.*)$"));
  static const QRegularExpression checked(QStringLiteral("^(\\s*)([*+-]\\s+\\[[xX]\\]\\s*)(.*)$"));
  ...
  if (match.hasMatch()) {
    return toggleDone
        ? match.captured(1) + QStringLiteral("- [x] ") + match.captured(3)
        : match.captured(1) + match.captured(3);
  }
  match = plain.match(line);
  if (!match.hasMatch()) return line;
  return match.captured(1) + (toggleDone ? QStringLiteral("- [x] ") : QStringLiteral("- [ ] ")) + match.captured(3);
}

MarkdownTableFormatResult MarkdownEdits::formatTableAtLine(
  ...
  const QStringList blockLines = lines.sliced(block->startLine, block->endLine - block->startLine + 1);
