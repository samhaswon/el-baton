...
void SyncController::sourceScrolled(int) {
  if (mode_ == SyncMode::Disabled) return;
  ...
  if (mode_ == SyncMode::Percentage) {
    ...
    const qsizetype span = std::max<qsizetype>(0, block->range.end - block->range.start);
    const bool interpolate = block->syncMode == QStringLiteral("interpolate") && span > 0;
    const double progress = interpolate
        ? clampProgress(static_cast<double>(offset - block->range.start) / span)
        : 0.0;
...
void SyncController::previewScrolled(const QJsonObject& position) {
  ...
  if (position.value("phase").toString() == QStringLiteral("end")) {
    if (owner_ == Owner::Preview) releaseOwner();
    return;
  }
  ...
  if (mode_ == SyncMode::Percentage) {
    const double percentage = clampProgress(position.value("percentage").toDouble());
    auto* bar = editor_->verticalScrollBar();
    targetVisibleLine = bar->minimum() +
        std::lround(percentage * (bar->maximum() - bar->minimum()));
  } else {
    const QString id = position.value("blockId").toString();
    const auto it = std::find_if(blocks_.cbegin(), blocks_.cend(),
                                 [&id](const RenderedBlock& block) {
                                   return block.id == id;
                                 });
    if (it == blocks_.cend()) {
    ...
    const qsizetype span = std::max<qsizetype>(0, it->range.end - it->range.start);
    const bool interpolate = it->syncMode == QStringLiteral("interpolate") && span > 0;
    const double progress = interpolate
        ? clampProgress(position.value("progress").toDouble())
        : 0.0;
  ...
  if (current == targetVisibleLine) {
    ...
    // wrapped continuations), the same units accepted by SCI_SETFIRSTVISIBLELINE.
    // Driving the scrollbar also guarantees a valueChanged notification, which
    // is synchronously suppressed here as the reciprocal programmatic event.
...
qsizetype SyncController::sourceOffsetAtViewport() const {
  if (lineOffsets_.isEmpty()) return 0;
  ...
  const int nextDisplay = documentLine + 1 < lineOffsets_.size()
...
int SyncController::visibleLineForSourceOffset(qsizetype offset) const {
  ...
  auto upper = std::upper_bound(lineOffsets_.cbegin(), lineOffsets_.cend(), offset);
  const int documentLine = std::max<int>(0, upper - lineOffsets_.cbegin() - 1);
  const qsizetype start = lineOffsets_.at(documentLine);
  const qsizetype end = documentLine + 1 < lineOffsets_.size()
      ? lineOffsets_.at(documentLine + 1)
      : textSize;
  const double fraction = end > start
  ...
  const int nextDisplay = documentLine + 1 < lineOffsets_.size()
      ? editor_->SendScintilla(QsciScintillaBase::SCI_VISIBLEFROMDOCLINE,
                               documentLine + 1)
      : firstDisplay + 1;
  return firstDisplay + std::lround(
      fraction * std::max(0, nextDisplay - firstDisplay - 1));
}

const RenderedBlock* SyncController::blockAtSourceOffset(qsizetype offset) const {
  if (blocks_.isEmpty()) return nullptr;
  auto upper = std::upper_bound(
      blocks_.cbegin(), blocks_.cend(), offset,
      [](qsizetype value, const RenderedBlock& block) {
        return value < block.range.start;
      });
  if (upper == blocks_.cbegin()) return &blocks_.first();
  --upper;
  // Zero-width/anchor-only blocks win at their exact source position.
  while (upper + 1 != blocks_.cend() && (upper + 1)->range.start == offset) ++upper;
  return &*upper;
}
...
double SyncController::sourcePercentage() const {
  ...
  return span > 0
      ? clampProgress(static_cast<double>(bar->value() - bar->minimum()) / span)
      : 0.0;
}
