...
const markdown_render_helpers_1 = require("../../src/common/markdown_render_helpers");
...
const renderMathPipeline = (markdown) => {
    ...
    }), rendered = markdown_render_helpers_1.default.renderKatexPlaceholders(withPlaceholders, placeholders, (tex, displayMode) => `<math mode="${displayMode ? 'display' : 'inline'}">${tex}</math>`);
    return markdown_render_helpers_1.default.restoreEscapedDollars(rendered);
};
/* TESTS */
(0, node_test_1.test)('katex placeholders: renders indexed placeholder payloads', () => {
    const placeholders = [
        { tex: 'x^2', displayMode: false },
        { tex: '\\frac{1}{2}', displayMode: true }
    ], html = 'A MDKATEXPLACEHOLDER0END B MDKATEXPLACEHOLDER1END', rendered = markdown_render_helpers_1.default.renderKatexPlaceholders(html, placeholders, (tex, displayMode) => `<katex mode="${displayMode ? 'display' : 'inline'}">${tex}</katex>`);
    assert.equal(rendered, 'A <katex mode="inline">x^2</katex> B <katex mode="display">\\frac{1}{2}</katex>');
});
(0, node_test_1.test)('katex placeholders: keeps unknown placeholders unchanged', () => {
...
(0, node_test_1.test)('katex parsing: leaves escaped dollar pairs untouched instead of treating them as math delimiters', () => {
    const markdown = 'Cost: \\$ $100 and display $$x^2$$ remain distinct', output = markdown_render_helpers_1.default.replaceMathDelimiters(markdown, (tex, displayMode) => `<math mode="${displayMode ? 'display' : 'inline'}">${tex}</math>`);
    assert.equal(output, 'Cost: \\$ $100 and display <math mode="display">x^2</math> remain distinct');
});
(0, node_test_1.test)('katex parsing: escaped currency placeholders survive multiple inline math spans on one line', () => {
    const markdown = 'Home price = (\\$ $\\times$ sqft) + (\\$ $\\times$ quality) - (\\$ $\\times$ Distance from water) + $\\pm$ (\\$ $\\times$ number of stories)', output = renderMathPipeline(markdown);
    assert.equal(output, 'Home price = ($ <math mode="inline">\\times</math> sqft) + ($ <math mode="inline">\\times</math> quality) - ($ <math mode="inline">\\times</math> Distance from water) + <math mode="inline">\\pm</math> ($ <math mode="inline">\\times</math> number of stories)');
});
(0, node_test_1.test)('katex parsing: preserves escaped currency placeholders until after math spans are rendered', () => {
    const markdown = '(\\$ $\\times$ sqft) + (\\$ $\\times$ quality)', escaped = markdown_render_helpers_1.default.replaceEscapedDollars(markdown), placeholders = [], withPlaceholders = markdown_render_helpers_1.default.replaceMathDelimiters(escaped, (tex, displayMode) => {
        const index = placeholders.push({ tex, displayMode }) - 1;
        return `MDKATEXPLACEHOLDER${index}END`;
    }), rendered = markdown_render_helpers_1.default.renderKatexPlaceholders(withPlaceholders, placeholders, tex => `<math>${tex}</math>`);
    assert.match(rendered, /MDESCAPEDDOLLARPLACEHOLDER <math>\\times<\/math>/);
    assert.doesNotMatch(rendered, /\(\$ <math>/);
    assert.equal(markdown_render_helpers_1.default.restoreEscapedDollars(rendered), '($ <math>\\times</math> sqft) + ($ <math>\\times</math> quality)');
});
(0, node_test_1.test)('katex parsing: renders the full repeated home-price expression without cross-consuming delimiters', () => {
    const markdown = 'Home price = (\\$ $\\times$ sqft) + (\\$ $\\times$ quality) - (\\$ $\\times$ Distance from water) - (\\$ $\\times$ distance to City Center) + (\\$ $\\times$ number of bedrooms) - (\\$ $\\times$ number of years since house was built) $\\pm$ (\\$ $\\times$ number of stories)', output = renderMathPipeline(markdown);
    assert.equal(output, 'Home price = ($ <math mode="inline">\\times</math> sqft) + ($ <math mode="inline">\\times</math> quality) - ($ <math mode="inline">\\times</math> Distance from water) - ($ <math mode="inline">\\times</math> distance to City Center) + ($ <math mode="inline">\\times</math> number of bedrooms) - ($ <math mode="inline">\\times</math> number of years since house was built) <math mode="inline">\\pm</math> ($ <math mode="inline">\\times</math> number of stories)');
});
(0, node_test_1.test)('macro placeholders: replaces supported markdown macros before parsing', () => {
    const markdown = 'Before\n\n[[@toc]]\n\n[[@pagebreak]]\n\nAfter', output = markdown_render_helpers_1.default.replaceMacroPlaceholders(markdown);
...
(0, node_test_1.test)('macro rendering: injects heading anchors and renders a linked table of contents', () => {
    const html = [
        '<p>MDMACROTOCPLACEHOLDER</p>',
        '<h2>Intro</h2>',
        '<h4>Details &amp; More</h4>',
        '<h2>Summary</h2>'
    ].join(''), output = markdown_render_helpers_1.default.renderMacros(html);
...
(0, node_test_1.test)('macro rendering: replaces pagebreak placeholders with print break markup', () => {
    const output = markdown_render_helpers_1.default.renderMacros('<p>MDMACROPAGEBREAKPLACEHOLDER</p>');
    assert.equal(output, '<hr class="pagebreak">');
});
(0, node_test_1.test)('mermaid block: encodes source and includes cached svg when present', () => {
    const source = 'graph TD\nA-->B', cachedSvg = '<svg><text>ok</text></svg>', html = markdown_render_helpers_1.default.renderMermaidBlock(source, cachedSvg);
...
(0, node_test_1.test)('mermaid block: returns no svg when cache is absent', () => {
    const html = markdown_render_helpers_1.default.renderMermaidBlock('flowchart LR\nA-->B');
    assert.match(html, /^<div class="mermaid"><code class="mermaid-source hidden">/);
    assert.doesNotMatch(html, /<svg/);
});
(0, node_test_1.test)('mermaid error: escapes the message and returns inline preview markup', () => {
    const html = markdown_render_helpers_1.default.renderMermaidError('Failed <bad> "input"');
    assert.equal(html, '<p class="mermaid-error text-warning">[mermaid error: Failed &lt;bad&gt; &quot;input&quot;]</p>');
});
(0, node_test_1.test)('mermaid external control: injects button into each mermaid container', () => {
    const input = '<div class="mermaid"></div><div class="mermaid"></div>', output = markdown_render_helpers_1.default.injectMermaidOpenExternal(input), count = (output.match(/mermaid-open-external/g) || []).length;
...
(0, node_test_1.test)('html sanitization: strips inline event handlers', () => {
    const html = '<img src="a.png" onerror="alert(1)" onclick=\'doBadThing()\'><div onmouseover=evil()>safe</div>', sanitized = sanitize(html);
    assert.equal(sanitized, '<img src="a.png"><div>safe</div>');
    assert.doesNotMatch(sanitized, /\son[a-z]+\s*=/i);
});
(0, node_test_1.test)('html sanitization: preserves safe tags and non-event attributes', () => {
    const html = '<table class="grid"><tr><td data-kind="x">cell</td></tr></table><img src="a.png" alt="A" class="thumb"><p id="ok">text</p>', sanitized = sanitize(html);
...
(0, node_test_1.test)('cmark html: preserves inline span markup and still parses inner markdown', () => {
...
(0, node_test_1.test)('cmark html: preserves aligned div blocks', () => {
...
(0, node_test_1.test)('cmark html: preserves raw table markup', () => {
    const markdown = [
        '<table>',
        '\t<tr>',
        '\t\t<th colspan="2"></th>',
...
(0, node_test_1.test)('cmark html + sanitizer: dangerous handlers are stripped while safe html remains', () => {
    const html = renderHtml('<img src="a.png" onerror="alert(1)"><table><tr><td>ok</td></tr></table>'), sanitized = sanitize(html);
...
(0, node_test_1.test)('cmark html: preserves iframes when raw html is allowed', () => {
...
(0, node_test_1.test)('cmark html + sanitizer: preserves safe iframe attrs and strips unsafe ones', () => {
    const html = renderHtml('<iframe width="100%" src="https://example.com/embed" onclick="evil()" srcdoc="<script>evil()</script>"></iframe>'), sanitized = sanitize(html);
...
(0, node_test_1.test)('html sanitization: strips unsafe URL schemes but preserves file and https urls', () => {
    const html = '<a href="javascript:alert(1)">bad</a><img src="file:///tmp/x.png"><iframe src="https://example.com/embed"></iframe>', sanitized = sanitize(html);
