...
let cachedLookup;
/* HELPERS */
const decodeEasterEggShortcode = (encodedShortcode, alertNumber) => {
    const normalizedShortcode = String(encodedShortcode || '').trim().toLowerCase(), shift = alertNumber % 26;
...
const getGemojiEntries = () => {
    try {
        const moduleExports = require('gemoji'), entries = moduleExports?.gemoji || moduleExports?.default || moduleExports;
        if (!Array.isArray(entries))
            return [];
        const normalized = [];
        for (let index = 0, length = entries.length; index < length; index++) {
            const entry = entries[index], names = Array.isArray(entry?.names) ? entry.names : [];
...
const Emoji = {
    ...
    replaceShortcodes(text, shouldSkip) {
        return text.replace(/:([a-z0-9_+\-]+):/gi, (match, shortcode, index, content) => {
            if (shouldSkip && shouldSkip(index, content))
                return match;
            return Emoji.get(shortcode) || match;
        });
    },
    getSuggestions(query = '', limit = 25) {
        const normalizedQuery = String(query || '').toLowerCase(), entries = getEntries(), startsWithMatches = entries.filter(entry => entry.shortcode.startsWith(normalizedQuery)), includesMatches = normalizedQuery ? entries.filter(entry => !entry.shortcode.startsWith(normalizedQuery) && entry.shortcode.includes(normalizedQuery)) : [];
        return startsWithMatches.concat(includesMatches).slice(0, Math.max(0, limit));
    },
    getAllShortcodes() {
        return getEntries().map(entry => entry.shortcode);
    },
    getEasterEgg(shortcode) {
