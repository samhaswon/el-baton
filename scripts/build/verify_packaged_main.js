...
const getArchivePath = contextOrAppOutDir => {
  ...
  const resourcesDir = typeof contextOrAppOutDir === 'string' ? null : contextOrAppOutDir.packager.getResourcesDir(appOutDir)
  const candidates = [
...
const assertArchiveEntry = ({ archivePath, entries, filePath, label = filePath, fallbackPaths = [] }) => {
  ...
  const similarEntries = Array.from(entries).filter(entry => entry.endsWith(basename)).slice(0, 8)
  const similarSuffix = similarEntries.length ? ` Similar archive entries: ${similarEntries.join(', ')}.` : ''
...
function verifyPackagedMain (contextOrAppOutDir) {
  ...
  const entries = getArchiveEntries(archivePath)
  const packageRaw = asar.extractFile(archivePath, 'package.json').toString('utf8')
  const packageJSON = JSON.parse(packageRaw)
  const rawMain = packageJSON.main
