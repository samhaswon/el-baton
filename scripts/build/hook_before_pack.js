...
const runNpmScript = ({ projectDir, script }) => {
  ...
  const result = cp.spawnSync(command, ['run', '-s', script], {
    cwd: projectDir,
    env: {
      ...process.env,
      NODE_ENV: 'production'
...
const ensureCompiledDist = ({ projectDir, sourceDistDir, snapshotDistDir, compiledSnapshotDistDir }) => {
  ...
  const missingMain = !fs.existsSync(compiledMainPath)
  const missingRenderer = !fs.existsSync(compiledRendererPath)
...
function beforePack (context) {
  ...
  const snapshotDistDir = getPackagedDistSnapshotDir(context)
  const compiledSnapshotDistDir = getCompiledDistSnapshotDir(projectDir)
