...
void SpellCheckerTest::identifiesProseButSkipsCode() {
  if (!qt_editor::SpellChecker::isAvailable()) QSKIP("Hunspell dictionary unavailable");
  const QVector<qt_editor::SpellingIssue> issues = qt_editor::SpellChecker::check(QStringLiteral(
...
void SpellCheckerTest::honorsAddedWords() {
  if (!qt_editor::SpellChecker::isAvailable()) QSKIP("Hunspell dictionary unavailable");
  QCOMPARE(qt_editor::SpellChecker::check(
      QStringLiteral("Codexword"), {QStringLiteral("codexword")}).size(), 0);
}

void SpellCheckerTest::reportsUtf8ByteOffsets() {
  if (!qt_editor::SpellChecker::isAvailable()) QSKIP("Hunspell dictionary unavailable");
  const QString source = QStringLiteral("Unicode café.\nA sentnce follows.\n");
  const QVector<qt_editor::SpellingIssue> issues = qt_editor::SpellChecker::check(source);
  const auto issue = std::find_if(issues.cbegin(), issues.cend(), [](const auto& candidate) {
