...
WatchedFileState readFileState(const QString& path) {
  ...
  if (file.open(QIODevice::ReadOnly)) {
    digest = QCryptographicHash::hash(file.readAll(), QCryptographicHash::Sha256);
  }
...
WorkspaceWatcher::WorkspaceWatcher(QObject* parent) : QObject(parent) {
  ...
  connect(&fileSystemWatcher_, &QFileSystemWatcher::directoryChanged,
          this, [this](const QString&) { scheduleScan(); });
...
void WorkspaceWatcher::setWorkspaceRoot(const QString& path) {
  ...
  if (workspaceRoot_ == normalized) return;
  const bool restart = active_;
  stop();
  workspaceRoot_ = normalized;
  if (restart && !workspaceRoot_.isEmpty()) start();
}
...
void WorkspaceWatcher::stop() {
  ...
  if (!watchedFiles.isEmpty()) fileSystemWatcher_.removePaths(watchedFiles);
...
WorkspaceSnapshot WorkspaceWatcher::takeSnapshot(QStringList* directories) const {
  WorkspaceSnapshot result;
  const QString rootPath = notesRoot();
  const QFileInfo rootInfo(rootPath);
  if (!rootInfo.isDir()) return result;

  if (directories != nullptr) directories->append(rootInfo.absoluteFilePath());
  QDirIterator iterator(rootPath, QDir::Dirs | QDir::Files | QDir::Readable | QDir::NoDotAndDotDot,
                        QDirIterator::Subdirectories);
  while (iterator.hasNext()) {
    const QString path = QFileInfo(iterator.next()).absoluteFilePath();
    const QFileInfo info(path);
    if (info.isDir()) {
      if (directories != nullptr) directories->append(path);
      continue;
    }
    if (!info.isFile() || !isSupportedNote(path)) continue;
    result.insert(path, readFileState(path));
  }
  return result;
}

QVector<WorkspaceChange> WorkspaceWatcher::compareSnapshots(
  ...
  for (auto iterator = before.cbegin(); iterator != before.cend(); ++iterator) {
    if (!after.contains(iterator.key())) removed.insert(iterator.key());
  }
  ...
  for (const QString& previousPath : sortedKeys(removed)) {
    ...
    if (renamedPath.isEmpty()) continue;
  ...
  for (const QString& path : sortedKeys(added)) changes.append({WorkspaceChangeKind::Added, path, {}});
  modified.sort(Qt::CaseSensitive);
  for (const QString& path : modified) changes.append({WorkspaceChangeKind::Modified, path, {}});
  for (const QString& path : sortedKeys(removed)) changes.append({WorkspaceChangeKind::Removed, path, {}});
  changes.append(renames);
  return changes;
}

void WorkspaceWatcher::scheduleScan() {
  if (active_) scanTimer_.start();
}
...
void WorkspaceWatcher::rebuildWatchPaths(const QStringList& directories) {
  ...
  if (!oldFiles.isEmpty()) fileSystemWatcher_.removePaths(oldFiles);
  const QStringList oldDirectories = fileSystemWatcher_.directories();
  if (!oldDirectories.isEmpty()) fileSystemWatcher_.removePaths(oldDirectories);

  QStringList existingDirectories;
  for (const QString& path : directories) {
    if (QFileInfo::exists(path)) existingDirectories.append(path);
  }
  if (!existingDirectories.isEmpty()) fileSystemWatcher_.addPaths(existingDirectories);

  QStringList files;
  files.reserve(snapshot_.size());
  for (auto iterator = snapshot_.cbegin(); iterator != snapshot_.cend(); ++iterator) files.append(iterator.key());
  if (!files.isEmpty()) fileSystemWatcher_.addPaths(files);
}
