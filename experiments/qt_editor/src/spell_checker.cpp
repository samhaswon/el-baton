...
struct Backend final {
  ...
  Backend() {
    ...
    if (base.isEmpty()) base = QStringLiteral("/usr/share/hunspell/en_US");
    ...
    if (QFileInfo(affix).isFile() && QFileInfo(dictionary).isFile()) {
      hunspell = std::make_unique<Hunspell>(affix.toLocal8Bit().constData(),
...
bool insideInlineCode(const QString& line, qsizetype position) {
  ...
  for (qsizetype index = 0; index < position; ++index) {
    if (line.at(index) == QLatin1Char('`') &&
        (index == 0 || line.at(index - 1) != QLatin1Char('\\'))) code = !code;
  }
  return code;
}
...
bool SpellChecker::isAvailable() {
  return backend().hunspell != nullptr;
}

QVector<SpellingIssue> SpellChecker::check(
    const QString& markdown,
    const QStringList& addedWords) {
  Backend& state = backend();
  QMutexLocker lock(&state.mutex);
  if (!state.hunspell) return {};

  QSet<QString> accepted;
  for (const QString& word : addedWords) accepted.insert(word.trimmed().toLower());
  static const QRegularExpression wordPattern(QStringLiteral("[A-Za-z][A-Za-z'’-]*"));
  static const QRegularExpression fencePattern(QStringLiteral("^\\s{0,3}(`{3,}|~{3,})"));
  ...
  for (const QString& line : lines) {
    ...
    if (!fenced) {
      ...
      while (matches.hasNext()) {
        ...
        if (word.size() <= 2 || QRegularExpression(QStringLiteral("^[A-Z]{2,}$")).match(word).hasMatch() ||
            accepted.contains(word.toLower()) || insideInlineCode(line, match.capturedStart())) continue;
        const QByteArray utf8 = word.toUtf8();
        if (state.hunspell->spell(utf8.toStdString())) continue;
        QStringList suggestions;
        const std::vector<std::string> candidates = state.hunspell->suggest(utf8.toStdString());
        ...
        issues.append({markdown.first(characterStart).toUtf8().size(), utf8.size(), word, suggestions});
      }
    }
    sourceOffset += line.size() + 1;
  }
