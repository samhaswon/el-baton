...
int ParseOptions(const Napi::Object &options) {
  ...
  for (const auto &[name, flag] : flags) {
    ...
    if (!value.IsEmpty() && value.ToBoolean().Value())
      result |= flag;
  }
...
bool AttachExtensions(const Napi::Object &options, cmark_parser *parser) {
  ...
  if (value.IsEmpty() || value.IsUndefined() || value.IsNull())
    return true;
  if (!value.IsObject()) {
    Napi::TypeError::New(options.Env(),
                         "The 'extensions' property must be an object")
  ...
  for (uint32_t index = 0; index < names.Length(); ++index) {
    ...
    if (!name_value.IsString() ||
        !extensions.Get(name_value).ToBoolean().Value())
      continue;
    const std::string name = name_value.As<Napi::String>().Utf8Value();
    cmark_syntax_extension *extension =
        cmark_find_syntax_extension(name.c_str());
    if (extension == nullptr) {
      Napi::Error::New(options.Env(), "Unknown cmark-gfm extension: " + name)
          .ThrowAsJavaScriptException();
      return false;
    }
...
Napi::Value RenderHtmlSync(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() < 1 || !info[0].IsString()) {
    Napi::TypeError::New(env, "Expected markdown to be a string")
        .ThrowAsJavaScriptException();
  ...
  if (parser == nullptr) {
    Napi::Error::New(env, "Unable to allocate cmark-gfm parser")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  ...
  if (html == nullptr) {
    Napi::Error::New(env, "Unable to render markdown")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
...
Napi::Value RenderCore(const Napi::CallbackInfo &info) {
  ...
  for (size_t index = 0; index < rendered.size();) {
    ...
    if (rendered.compare(index, kCodeOpen.size(), kCodeOpen) == 0) {
      ...
      const size_t code_close =
          code_open_end == std::string::npos
              ? std::string::npos
              : rendered.find(kCodeClose, code_open_end + 1);
      if (code_close != std::string::npos) {
        const size_t slot = slots.size();
        const size_t full_end = code_close + kCodeClose.size();
        slots.push_back(
            {"code", 0,
             rendered.substr(index + kCodeOpen.size(),
                             code_open_end - index - kCodeOpen.size()),
             rendered.substr(code_open_end + 1, code_close - code_open_end - 1),
             rendered.substr(index, full_end - index)});
    ...
    while (digits < rendered.size() &&
           std::isdigit(static_cast<unsigned char>(rendered[digits])))
      ++digits;
    if (digits == index + kPrefix.size() ||
        rendered.compare(digits, 3, "END") != 0) {
      output.push_back(rendered[index++]);
      continue;
    }
    size_t math_index = 0;
    if (!ParseSizeT(
            std::string_view(rendered).substr(index + kPrefix.size(),
                                              digits - index - kPrefix.size()),
            &math_index)) {
      output.push_back(rendered[index++]);
      continue;
    }
  ...
  for (uint32_t index = 0; index < slots.size(); ++index) {
    ...
    if (slots[index].type == "katex")
      slot.Set("index", static_cast<double>(slots[index].math_index));
    else {
...
Napi::Value Finalize(const Napi::CallbackInfo &info) {
  const Napi::Env env = info.Env();
  if (info.Length() < 1 || !info[0].IsString()) {
    Napi::TypeError::New(env, "Expected rendered template to be a string")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }

  const std::string input = info[0].As<Napi::String>().Utf8Value();
  const Napi::Array slots = (info.Length() > 1 && info[1].IsArray())
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    while (digits < input.size() &&
      ...
      ++digits;
    if (digits == index + kPrefix.size() ||
        input.compare(digits, 3, "END") != 0) {
      output.push_back(input[index++]);
      continue;
    }
    size_t slot = 0;
    if (!ParseSizeT(
            std::string_view(input).substr(index + kPrefix.size(),
                                           digits - index - kPrefix.size()),
            &slot)) {
    ...
    const Napi::Value replacement = slot < slots.Length()
                                        ? slots.Get(static_cast<uint32_t>(slot))
                                        : env.Undefined();
    if (replacement.IsString())
      output += replacement.As<Napi::String>().Utf8Value();
    else
      output.append(input, index, digits + 3 - index);
    index = digits + 3;
  }
...
Napi::Value ReplaceMacroPlaceholders(const Napi::CallbackInfo &info) {
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    if (remaining >= 8 && input[index] == '[' && input[index + 1] == '[' &&
        input[index + 2] == '@') {
...
const std::string_view FindEmoji(const std::string &name) {
  ...
  for (size_t probes = 0; probes < markdown_native::kEmojiEntries.size();
       ++probes) {
  ...
  for (size_t index = 0; index < input.size();) {
    if (line_start) {
      ...
      while (marker < input.size() &&
        ...
        ++marker;
      if (marker < input.size() &&
          (input[marker] == '`' || input[marker] == '~')) {
        size_t end = marker;
        while (end < input.size() && input[end] == input[marker])
          ++end;
        ...
        if (!in_fence && length >= 3) {
          ...
          fence_length = length;
          fence_line = true;
        } else if (in_fence && input[marker] == fence_char &&
                   length >= fence_length) {
          in_fence = false;
          fence_line = true;
        }
    ...
    if (input[index] == '`') {
      ...
      output.append(input, index, count);
      index = end;
      line_start = false;
      continue;
    }
...
Napi::Value ReplaceWikilinks(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() < 5 || !info[0].IsString() || !info[1].IsString() ||
    ...
    Napi::TypeError::New(
        env, "Expected markdown, token, extension, note regex, and flags")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  const std::string input = info[0].As<Napi::String>().Utf8Value();
  ...
  const std::regex note_regex(regex_source, flags.find('i') != std::string::npos
                                                ? std::regex::icase
  ...
  for (size_t index = 0; index < input.size();) {
    if (line_start) {
      ...
      if (marker < input.size() &&
        ...
        while (end < input.size() && input[end] == input[marker])
          ++end;
        if (!in_fence && end - marker >= 3) {
          in_fence = true;
          fence_char = input[marker];
          fence_length = end - marker;
          fence_line = true;
        } else if (in_fence && input[marker] == fence_char &&
                   end - marker >= fence_length) {
          in_fence = false;
          fence_line = true;
        }
      }
    ...
    if (!in_code && input.compare(index, 2, "[[") == 0) {
      ...
      if (close != std::string::npos) {
        ...
        const std::string note = separator == std::string::npos
                                     ? title
                                     : inner.substr(separator + 1);
        if (!title.empty() && !note.empty()) {
          output += "<a href=\"" + token + "/" + note +
                    (std::regex_search(note, note_regex) ? "" : extension) +
                    "\">" + title + "</a>";
          index = close + 2;
          line_start = false;
          continue;
...
Napi::Value EncodeSpecialLinks(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() < 4 || !info[0].IsString() || !info[1].IsString() ||
    ...
    Napi::TypeError::New(env, "Expected markdown and token strings")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  const std::string input = info[0].As<Napi::String>().Utf8Value();
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    if (!in_code && input[index] == '[') {
      ...
      const size_t target_end = label_end == std::string::npos
                                    ? std::string::npos
                                    : input.find(')', label_end + 2);
      if (target_end != std::string::npos) {
        const std::string target =
            input.substr(label_end + 2, target_end - label_end - 2);
        bool special = false;
        for (const std::string &token : tokens) {
          if (!token.empty() && target.rfind(token + "/", 0) == 0) {
            special = true;
            break;
          }
        }
...
Napi::Value ReplaceSuperscriptSubscript(const Napi::CallbackInfo &info) {
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    while (end < input.size() && input[end] != marker && input[end] != '\n')
      ++end;
    if (end >= input.size() || input[end] != marker || end == index + 1 ||
        std::isspace(static_cast<unsigned char>(input[index + 1])) ||
        std::isspace(static_cast<unsigned char>(input[end - 1]))) {
      output.push_back(input[index++]);
      continue;
    }
    if (superscript && index > 0 && input[index - 1] == '^') {
      output.push_back(input[index++]);
      continue;
    }
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    for (; close < input.size(); ++close) {
      if (!display && input[close] == '\n')
        break;
      if (input[close] != '$' || IsEscaped(input, close))
        continue;
      if (display) {
        if (close + 1 < input.size() && input[close + 1] == '$')
          break;
      } else if ((close == 0 || input[close - 1] != '$') &&
                 (close + 1 == input.size() || input[close + 1] != '$'))
        break;
    }
    if (close >= input.size() ||
        (!display && (input[close] != '$' || input[close] == '\n')) ||
        (display && (close + 1 >= input.size() || input[close + 1] != '$'))) {
      output.push_back(input[index++]);
      continue;
    }
    const std::string tex =
        input.substr(index + delimiter, close - index - delimiter);
    if (tex.empty()) {
      output.append(input, index, close + delimiter - index);
...
Napi::Value PrepareMath(const Napi::CallbackInfo &info) {
  ...
  for (size_t index = 0; index < escaped.size();) {
    if (escaped[index] != '$' || IsEscaped(escaped, index)) {
      extracted.push_back(escaped[index++]);
      continue;
    }
    const bool display =
        index + 1 < escaped.size() && escaped[index + 1] == '$';
    const size_t delimiter = display ? 2 : 1;
    size_t close = index + delimiter;
    for (; close < escaped.size(); ++close) {
      if (!display && escaped[close] == '\n')
        break;
      if (escaped[close] != '$' || IsEscaped(escaped, close))
        continue;
      if (display
              ? close + 1 < escaped.size() && escaped[close + 1] == '$'
              : (close == 0 || escaped[close - 1] != '$') &&
                    (close + 1 == escaped.size() || escaped[close + 1] != '$'))
        break;
    }
    if (close >= escaped.size() || (!display && escaped[close] != '$') ||
        (display &&
         (close + 1 >= escaped.size() || escaped[close + 1] != '$'))) {
      extracted.push_back(escaped[index++]);
      continue;
    ...
    const std::string tex =
        escaped.substr(index + delimiter, close - index - delimiter);
    if (tex.empty()) {
      extracted.append(escaped, index, close + delimiter - index);
      index = close + delimiter;
      continue;
    }
    const size_t slot = payloads.size();
  ...
  for (size_t index = 0; index < extracted.size();) {
    ...
    const bool superscript = marker == '^';
    const bool subscript = marker == '~' && (index + 1 >= extracted.size() ||
                                             extracted[index + 1] != '~');
    const bool escaped_marker = index > 0 && extracted[index - 1] == '\\';
    const bool doubled = index > 0 && extracted[index - 1] == marker;
    if ((!superscript && !subscript) || escaped_marker || doubled) {
      output.push_back(extracted[index++]);
      continue;
    }
    size_t end = index + 1;
    while (end < extracted.size() && extracted[end] != marker &&
           extracted[end] != '\n')
      ++end;
    if (end >= extracted.size() || extracted[end] != marker ||
        end == index + 1 ||
        std::isspace(static_cast<unsigned char>(extracted[index + 1])) ||
        std::isspace(static_cast<unsigned char>(extracted[end - 1]))) {
      output.push_back(extracted[index++]);
      continue;
    }
    output += superscript ? "<sup>" : "<sub>";
...
Napi::Value NumberCheckboxes(const Napi::CallbackInfo &info) {
  const Napi::Env env = info.Env();
  if (info.Length() < 1 || !info[0].IsString()) {
    Napi::TypeError::New(env, "Expected html to be a string")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  const std::string input = info[0].As<Napi::String>().Utf8Value();
  ...
  while (offset < input.size()) {
    ...
    if (disabled != std::string::npos) {
      ...
      if (value_end != std::string::npos)
        tag.erase(disabled, value_end - disabled + 1);
    ...
    if (task_item != std::string::npos)
      tag.replace(task_item, 4, "<li class=\"task-list-item\">");
...
Napi::Value AddBlankTargets(const Napi::CallbackInfo &info) {
  ...
  while (offset < input.size()) {
    ...
    if (end == std::string::npos || href == std::string::npos || href > end) {
      output.append(input, offset,
                    end == std::string::npos ? std::string::npos
                                             : end + 1 - offset);
      offset = end == std::string::npos ? input.size() : end + 1;
      continue;
    }
    const size_t value_start = href + 6;
    const size_t value_end = input.find('"', value_start);
    if (value_end == std::string::npos || value_end > end ||
        (value_start < input.size() && input[value_start] == '#')) {
      output.append(input, offset, end + 1 - offset);
      offset = end + 1;
      continue;
    }
...
Napi::Value NormalizeLinkProtocols(const Napi::CallbackInfo &info) {
  ...
  while (offset < input.size()) {
    ...
    const size_t value_start = href + 6,
                 value_end = input.find('"', value_start);
    if (value_end == std::string::npos || value_end > end) {
      output.append(input, offset, end + 1 - offset);
      offset = end + 1;
      continue;
    ...
    const std::string value =
        input.substr(value_start, value_end - value_start);
    output.append(input, offset, value_start - offset);
    if (!value.empty() && value[0] != '#' && !HasProtocol(value))
      output += "https://";
    output += value;
...
Napi::Value WrapCodeBlocks(const Napi::CallbackInfo &info) {
  ...
  constexpr std::string_view kWrapper =
      "<div class=\"copy-wrapper\"><div class=\"copy\" title=\"Copy code to "
      "clipboard\"><i class=\"icon small\">content_copy</i></div>";
  while (offset < input.size()) {
...
Napi::Value InjectDiagramControls(const Napi::CallbackInfo &info) {
  ...
  ReplaceAll(&output, "<div class=\"mermaid\">",
             "<div class=\"mermaid\"><div class=\"mermaid-open-external\" "
             "title=\"Open in Separate Window\"><i class=\"icon "
             "small\">open_in_new</i></div>");
  ReplaceAll(&output, "<div class=\"plantuml\">",
             "<div class=\"plantuml\"><div class=\"plantuml-open-external "
             "hidden\" title=\"Open External Diagram\"><i class=\"icon "
             "small\">open_in_new</i></div>");
  return Napi::String::New(env, output);
}
...
std::string DecodeHtmlEntitiesForUrl(const std::string &input) {
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    if (entity.size() > 1 && entity[0] == '#') {
      ...
      if (first_digit < entity.size()) {
        ...
        for (size_t digit = first_digit; digit < entity.size(); ++digit) {
          ...
          if (parsed < 0 || parsed >= base ||
              value > 0x10ffffu / static_cast<uint32_t>(base)) {
            decoded = false;
            break;
          }
      ...
      value = ':';
      decoded = true;
    } else if (entity == "tab" || entity == "Tab") {
      value = '\t';
      decoded = true;
    } else if (entity == "newline" || entity == "NewLine") {
      value = '\n';
      decoded = true;
    } else if (entity == "nbsp") {
      value = 0xa0;
...
std::string DecodePercentEscapes(const std::string &input) {
  ...
  for (size_t index = 0; index < input.size();) {
    if (input[index] == '%' && index + 2 < input.size()) {
      const int high = DigitValue(input[index + 1]),
                low = DigitValue(input[index + 2]);
      if (high >= 0 && low >= 0) {
...
bool IsUnsafeUrlProtocol(const std::string &raw_value) {
  ...
  if (normalized.rfind("javascript:", 0) == 0 ||
      normalized.rfind("vbscript:", 0) == 0)
    return true;
  return normalized.rfind("data:", 0) == 0 &&
         !(normalized.rfind("data:image/png;", 0) == 0 ||
           normalized.rfind("data:image/gif;", 0) == 0 ||
           normalized.rfind("data:image/jpeg;", 0) == 0 ||
           normalized.rfind("data:image/webp;", 0) == 0);
}
...
bool StartsWithInsensitive(const std::string &input, size_t offset,
                           std::string_view value) {
  return offset + value.size() <= input.size() &&
         EqualsInsensitive(std::string_view(input).substr(offset, value.size()),
                           value);
}

bool IsBlockedElement(std::string_view name) {
  return EqualsInsensitive(name, "script") ||
         EqualsInsensitive(name, "style") || EqualsInsensitive(name, "title") ||
         EqualsInsensitive(name, "textarea") ||
         EqualsInsensitive(name, "xmp") || EqualsInsensitive(name, "noembed") ||
         EqualsInsensitive(name, "noframes") ||
         EqualsInsensitive(name, "plaintext");
}

bool IsUrlAttribute(std::string_view name) {
  return EqualsInsensitive(name, "href") || EqualsInsensitive(name, "src") ||
         EqualsInsensitive(name, "xlink:href") ||
         EqualsInsensitive(name, "action") ||
         EqualsInsensitive(name, "formaction") ||
         EqualsInsensitive(name, "poster");
}
...
std::string SanitizeTag(const std::string &tag) {
  if (tag.size() < 3 || tag.back() != '>' || tag[1] == '/' || tag[1] == '!' ||
      tag[1] == '?')
    return tag;
  size_t index = 1;
  while (index < tag.size() &&
    ...
    ++index;
  while (index < tag.size() &&
         (std::isalnum(static_cast<unsigned char>(tag[index])) ||
          tag[index] == ':' || tag[index] == '-'))
    ++index;
  ...
  while (index < body_end) {
    ...
    if (index >= body_end || tag[index] == '/') {
      output.append(tag, attribute_start, body_end - attribute_start);
      break;
    }
    ...
    if (index < body_end && tag[index] == '=') {
      ...
      if (index < body_end && (tag[index] == '\'' || tag[index] == '"')) {
        ...
        while (index < body_end && tag[index] != quote)
          ++index;
        if (value_start > index || index > body_end)
          return tag;
    ...
    const bool event = name.size() > 2 && (name[0] == 'o' || name[0] == 'O') &&
                       (name[1] == 'n' || name[1] == 'N') &&
                       std::isalpha(static_cast<unsigned char>(name[2]));
    const bool remove = event || EqualsInsensitive(name, "srcdoc") ||
                        (IsUrlAttribute(name) && IsUnsafeUrlProtocol(value));
    if (attribute_start > index || index > body_end)
      return tag;
    if (!remove)
      output.append(tag, attribute_start, index - attribute_start);
  }
  output.push_back('>');
  return output;
}
  ...
  for (size_t index = 0; index < input.size();) {
    ...
    const size_t name_end = [&] {
      ...
      while (cursor < tag_end &&
             (std::isalnum(static_cast<unsigned char>(input[cursor])) ||
              input[cursor] == ':' || input[cursor] == '-'))
        ++cursor;
      return cursor;
    }();
    const std::string_view name(input.data() + name_start,
                                name_end - name_start);
    if (IsBlockedElement(name)) {
      ...
      while (close_start < input.size() &&
             !StartsWithInsensitive(input, close_start, needle))
        ++close_start;
...
std::string SlugifyHeading(const std::string &text,
...
std::string RenderMacroToc(const std::vector<Heading> &headings) {
  if (headings.empty())
    return "";
  std::string output = "<div class=\"macro-toc\"><p "
  ...
  for (const Heading &heading : headings) {
    ...
    output += "<li><a class=\"toc-item\" href=\"#" + heading.id + "\">" +
...
Napi::Value RenderMacros(const Napi::CallbackInfo &info) {
  ...
  const std::string input = info[0].As<Napi::String>().Utf8Value();
  static const std::regex headings("<h([1-6])(\\s[^>]*)?>([\\s\\S]*?)</h\\1>",
                                   std::regex::icase);
  static const std::regex existing_id("\\sid=\\\"([^\\\"]+)\\\"",
                                      std::regex::icase);
  ...
  size_t offset = 0;
  for (std::sregex_iterator match(input.begin(), input.end(), headings), end;
       match != end; ++match) {
    const std::smatch &current = *match;
    with_anchors.append(input, offset,
                        static_cast<size_t>(current.position()) - offset);
    ...
    if (text.empty()) {
      ...
      const std::string id =
          has_id ? id_match[1].str() : SlugifyHeading(text, &slug_counts);
      collected.push_back({id, level, text});
      with_anchors += "<h" + std::to_string(level) + attrs +
                      (has_id ? "" : " id=\"" + id + "\"") + ">" + inner_html +
  ...
  ReplaceAll(&with_anchors, "<p>MDMACROTOCPLACEHOLDER</p>",
             RenderMacroToc(collected));
  ReplaceAll(&with_anchors, "<p>MDMACROPAGEBREAKPLACEHOLDER</p>",
             "<hr class=\"pagebreak\">");
...
Napi::Value SanitizeStaticHtml(const Napi::CallbackInfo &info) {
  ...
  if (info.Length() < 2 || !info[0].IsString()) {
    Napi::TypeError::New(env, "Expected html and enabled flag")
        .ThrowAsJavaScriptException();
    return env.Undefined();
  }
  ...
  return Napi::String::New(
      env, SanitizeHtmlOnePass(info[0].As<Napi::String>().Utf8Value()));
}
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  ...
  exports.Set("replaceMacroPlaceholders",
              Napi::Function::New(env, ReplaceMacroPlaceholders));
  exports.Set("replaceEscapedDollars",
              Napi::Function::New(env, ReplaceEscapedDollars));
  exports.Set("replaceEmojiShortcodes",
              Napi::Function::New(env, ReplaceEmojiShortcodes));
  exports.Set("replaceWikilinks", Napi::Function::New(env, ReplaceWikilinks));
  exports.Set("encodeSpecialLinks",
              Napi::Function::New(env, EncodeSpecialLinks));
  exports.Set("replaceSuperscriptSubscript",
              Napi::Function::New(env, ReplaceSuperscriptSubscript));
  exports.Set("extractMathDelimiters",
              Napi::Function::New(env, ExtractMathDelimiters));
  ...
  exports.Set("normalizeLinkProtocols",
              Napi::Function::New(env, NormalizeLinkProtocols));
  exports.Set("wrapCodeBlocks", Napi::Function::New(env, WrapCodeBlocks));
  exports.Set("injectDiagramControls",
              Napi::Function::New(env, InjectDiagramControls));
  exports.Set("renderMacros", Napi::Function::New(env, RenderMacros));
  exports.Set("sanitizeStaticHtml",
              Napi::Function::New(env, SanitizeStaticHtml));
