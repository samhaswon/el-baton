...
private slots:
  ...
  void normalizesWrappers() {
    ...
    QCOMPARE(PlantUmlRenderer::normalizeSource(
  ...
  void buildsReferenceCompatibleRemoteUrlsAndEncoding() {
    QCOMPARE(PlantUmlRenderer::normalizeServerUrl(
                 QStringLiteral("example.com/plantuml/")),
             QStringLiteral("https://example.com/plantuml"));
    QCOMPARE(PlantUmlRenderer::buildRemoteRenderUrl(
                 QStringLiteral("https://example.com/plantuml")),
             QStringLiteral("https://example.com/plantuml/svg"));
    QCOMPARE(PlantUmlRenderer::buildRemoteSvgUrl(
                 QStringLiteral("https://example.com/plantuml/svg/old"),
    ...
    QVERIFY(QRegularExpression(QStringLiteral("^[0-9A-Za-z_-]+$"))
                .match(encoded)
                .hasMatch());
  }
  ...
  void rendersWithPinnedLocalJar() {
    ...
    const QJsonObject result =
        batch.value(QStringLiteral("results")).toArray().at(0).toObject();
  ...
  void remotePostOverridesSuccessfulLocalRender() {
    ...
    connect(&server, &QTcpServer::newConnection, this, [&] {
      ...
      connect(socket, &QTcpSocket::readyRead, socket, [&, socket] {
        ...
        const QRegularExpressionMatch match = contentLength.match(
            QString::fromLatin1(requestBytes.left(headersEnd)));
        if (!match.hasMatch() ||
            requestBytes.size() < headersEnd + 4 + match.captured(1).toInt())
          return;
        const QByteArray svg = "<svg "
                               "xmlns=\"http://www.w3.org/2000/"
                               "svg\"><text>remote-marker</text></svg>";
    ...
    const QJsonObject result = spy.takeFirst()
                                   ...
                                   .at(0)
                                   .toObject();
    QCOMPARE(result.value(QStringLiteral("origin")).toString(),
             QStringLiteral("remote"));
    QCOMPARE(result.value(QStringLiteral("localStatus")).toString(),
             QStringLiteral("ok"));
    QCOMPARE(result.value(QStringLiteral("remoteStatus")).toString(),
             QStringLiteral("ok"));
    QVERIFY(result.value(QStringLiteral("svg"))
                .toString()
                .contains(QStringLiteral("remote-marker")));
  ...
  void remoteGetFallbackUsesEncodedDiagramUrl() {
    ...
    connect(&server, &QTcpServer::newConnection, this, [&] {
      ...
      connect(socket, &QTcpSocket::readyRead, socket, [&, socket] {
        ...
        if (!request.contains("\r\n\r\n"))
          return;
        requestLines.append(
            QString::fromLatin1(request.first(request.indexOf("\r\n"))));
        if (request.startsWith("POST ")) {
          ...
          const QByteArray svg = "<svg "
                                 "xmlns=\"http://www.w3.org/2000/"
                                 "svg\"><text>get-fallback</text></svg>";
          socket->write("HTTP/1.1 200 OK\r\nContent-Type: "
                        "image/svg+xml\r\nContent-Length: " +
                        QByteArray::number(svg.size()) +
                        "\r\nConnection: close\r\n\r\n" + svg);
    ...
    QVERIFY(result.value(QStringLiteral("svg"))
                .toString()
                .contains(QStringLiteral("get-fallback")));
    QCOMPARE(requestLines.size(), 2);
    QVERIFY(
        requestLines.at(0).startsWith(QStringLiteral("POST /plantuml/svg ")));
    QVERIFY(QRegularExpression(
                QStringLiteral("^GET /plantuml/svg/[0-9A-Za-z_-]+ HTTP/1\\.1$"))
