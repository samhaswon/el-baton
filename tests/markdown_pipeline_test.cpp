...
void compareRenderedSemantics(const RenderResult& actual, const RenderResult& expected) {
...
QVector<RenderedBlock> applyPatch(const QVector<RenderedBlock>& previous,
  ...
  for (const RenderedBlock& block : previous) byId.insert(block.id, block);
  for (const QString& id : patch.removedBlockIds) byId.remove(id);
  for (const RenderedBlock& block : patch.blocks) byId.insert(block.id, block);
...
QString largeTestDocument() {
  ...
  for (int section = 0; section < 900; ++section) {
    markdown += QStringLiteral(
        "## Generated section %1\n\n"
        "Paragraph %1 discusses equilibrium, temperature, and a :cat: marker. "
        "It links to [[Generated note %1]] and contains `inline code %1`.\n\n"
        "| quantity | value |\n"
        "| --- | ---: |\n"
        "| sample | %1 |\n\n"
        "- [ ] observation %1\n"
...
class MarkdownPipelineTest final : public QObject {
 ...
 private slots:
  ...
  void fullLargeDocumentRenderIsTheCorrectnessOracle() {
    ...
    QVERIFY(html.contains(QStringLiteral("<h1 id=\"1-test-note\">1 Test Note</h1>")));
    ...
    QVERIFY(html.contains(QStringLiteral("<h1 id=\"some-heading\">Some Heading</h1>")));
    QVERIFY(html.contains(QStringLiteral("<p>The user is editing this line</p>")));
    QVERIFY(html.contains(QStringLiteral("<details data-nth=\"0\" open>")));
    QVERIFY(html.contains(QStringLiteral("qt-mermaid")));
    QVERIFY(html.contains(QStringLiteral("data-tex=\"\\ce{CO2 + C -&gt; 2CO}")));
  ...
  void nonLinearIncrementalConstructionMatchesFullRenderingAndPatches() {
    ...
    const qsizetype generatedStart = complete.indexOf(QStringLiteral("## Generated section 0"));
    const qsizetype section200 = complete.indexOf(QStringLiteral("## Generated section 200"));
    const qsizetype section450 = complete.indexOf(QStringLiteral("## Generated section 450"));
    const qsizetype section700 = complete.indexOf(QStringLiteral("## Generated section 700"));
    ...
    const QString early = complete.sliced(generatedStart, section200 - generatedStart);
    ...
    for (qsizetype index = 0; index < states.size(); ++index) {
      const RenderResult patch = incremental.render(states.at(index), static_cast<quint64>(index + 1));
      simulatedDom = applyPatch(simulatedDom, patch);
      const RenderResult full = renderFresh(states.at(index), static_cast<quint64>(index + 1));
      ...
      for (qsizetype block = 0; block < simulatedDom.size(); ++block) {
        ...
        QCOMPARE(simulatedDom.at(block).range.start, patch.allBlocks.at(block).range.start);
        QCOMPARE(simulatedDom.at(block).range.end, patch.allBlocks.at(block).range.end);
      }
      if (index > 0) {
  ...
  void incrementalEditsRespectRawHtmlAndDetailsScope() {
    ...
    markdown.replace(QStringLiteral("<div align=\"center\">"),
    ...
    markdown.replace(QStringLiteral("A paragraph inside generated details 450."),
