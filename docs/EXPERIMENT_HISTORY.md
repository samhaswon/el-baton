...
The retired `experiments/qt_editor` directory began as an isolated comparison
of QScintilla plus QWebEngineView against Monaco/Electron. The experiment
established a viable native editing and rendering path; its code now lives in
the root `src/` and `tests/` directories and is built as `el_baton_core`.

This document preserves the focused rendering benchmark and protocol notes.
The Electron implementation is retained under `reference/`. Current application
status belongs in [PORT_STATUS.md](PORT_STATUS.md).
...
Run the application with a document path:

```bash
build/native/bin/el-baton resources/demo_data/seed/notes/\
...
```bash
el-baton DOC --sync off --no-katex --no-mermaid --patch full
el-baton DOC --sync percentage --no-katex --no-mermaid --patch blocks
el-baton DOC --sync semantic --no-katex --no-mermaid --patch blocks
el-baton DOC --sync semantic --patch blocks
el-baton DOC --sync semantic --patch full
```
